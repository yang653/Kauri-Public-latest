...
//     }
    ...
    /* commit requires direct parent */
    if (blk2->parents[0] != blk1) {
        block_t b;
        std::vector<block_t> c;
        for(b=blk2;b->height>blk1->height;b=b->parents[0]){
            c=FindEquivProof(blk1,b,b->NVset);
            if(c.size()!=0){
                return;
            }
        }
    ...
    std::vector<block_t> commit_queue;
    block_t b;
    for (b = blk1; b->height > b_exec->height; b = b->parents[0])
    {
        commit_queue.push_back(b);
...
std::vector<block_t> HotStuffCore::FindEquivProof(const block_t &Bparents, const block_t &Banc, std::unordered_map<NewView, ReplicaID> &Nvset)
{
    std::vector<block_t> c;
    for(auto newview:Nvset){
        block_t blk=newview.first->Vote_Req.blk;
   ...
   // std::vector<NewView> NVset;
    if (piped_queue.empty()) {
        ...
        if (newParents[0]->height <= piped_block->height) {
            ...
            newParents.insert(newParents.begin(), piped_block);//插入队列区块,父母区块的第0块是高度最高的块
        }
    ...
    b_normal_height = bnew->get_height();//为协议正常进行的区块高度
...
void HotStuffCore::on_receive_vote(const Vote &vote) {
    ...
    if (qc->has_n(config.nmajority))/*计算qc是否达到阈值*/
    {
        qc->compute();
        update_hqc(blk, qc);
        on_qc_finish(blk);
