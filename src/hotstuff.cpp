...
void HotStuffBase::newview_handler(MsgNewview &&msg, const Net::conn_t &)
{
    ...
    if(id == pmaker->get_proposer()&&msg.newview.viewNum>vheight){
        ...
        if(NVset_t.size()>=config.nmajority){
            ...
            for(const auto& new_view: NVset_t){
                ...
                blks.insert(value.Vote_Req.blk);
            }
            ...
            quorum_cert_bt QCanc=highblk->qc;
            block_t Banc;
            if(QCanc){
            ...
            std::unordered_map<int,block_t> numvote;
            for(const auto& newview:NVset_t){
                block_t respBlk=storage->find_blk(newview.first.Vote_Resp.blk_hash);
                if(respBlk==nullptr){
                    throw std::runtime_error("block referred by qc not fetched");
                }
            ...
            for(auto num:numvote){
                if(num.first()>=config.nmajority){
                    block_t blk=num.second();
