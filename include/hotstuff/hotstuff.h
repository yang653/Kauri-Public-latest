...
#ifndef _HOTSTUFF_CORE_H//如果没有定义_HOTSTUFF_CORE_H那么将执行一下全部代码
...
struct MsgPropose {
    ...
    Proposal proposal;//提案信息
    MsgPropose(const Proposal &);
    /** Only move the data to serialized, do not parse immediately. */
    MsgPropose(DataStream &&s): serialized(std::move(s)) {}//构造函数，引用参数s转移给成员变量serialized
...
class FetchContext: public promise_t {
    ...
    public:
    FetchContext(const FetchContext &) = delete;//禁止拷贝创建对象，禁止通过运算符重载=来创建对象
    FetchContext &operator=(const FetchContext &) = delete;
    FetchContext(FetchContext &&other);//可以用右值引用创建对象
...
class HotStuffBase: public HotStuffCore {
    ...
    uint64_t fetched;
    uint64_t delivered;
    mutable uint64_t nsent;//可在const成员函数内部被更改
    ...
    inline void newview_handler(MsgNewview &&, const Net::conn_t &);
