...
class HotStuffCore {
    ...
    /* === async event queues === */
   // std::vector<vote> vote_resp;
    std::unordered_map<NewView,ReplicaID> NVset_t;/*newview消息*/
...
bool rdy = false;
    ...
    std::vector<block_t> FindEquivProof(const block_t &Bparents,const block_t &Banc,std::unordered_map<NewView,ReplicaID> &Nvset);
